...
export default function NewQuickView({
    ...
    return (
      <>
        ...
        <Button
          onClick={changePhase}
          fullWidth={true}
          isLoading={changingPhase || validating}
          help={`${
  ...
  const phaseCallback = changePhase => {
    if (changePhase) {
      if (notLastPhase) {
        changePhase()
      } else {
  ...
  return (
    <div>
      <div className="quicknav-container">
        ...
        <div className="quicknav-onhold-form">{renderCheckBox()}</div>
        <ConfirmModal
